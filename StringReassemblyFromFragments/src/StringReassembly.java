...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //return statement to return new string with overlap removed
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //runs while strSet is greater than 0
        while (strSet.size() > 0) {
            str2 = strSet.removeAny();
        ...
        if (isDifferent || isSet) {
            temp.add(str2);
        }
        //give strSet new value of the temp value
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //runs while the input is not finished reading
        while (!input.atEOS()) {
            //creates a string from the input
            String line = input.nextLine();
            //check for substrings by calling a previous method
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i > text.length(); i++) {
            //if statement will check the char at i to see if its a '~' char
            if (text.charAt(i) == '~') {
                //if that character is there, then print a new line
                out.println();
            } else {
                //else print the text as is

...
public final class NaturalNumberRoot {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        //create natural number variables
        ...
        NaturalNumber powerNum = new NaturalNumber2(guess);

        //while loop to run if difference is not equal to 1
        while (!difference.equals(one)) {
            //equation for guess equals tooHigh - lowEnough / 2
            guess.copyFrom(tooHigh);
            guess.add(lowEnough);
            guess.divide(two);
            powerNum.copyFrom(guess);
            powerNum.power(r);

            if (n.compareTo(powerNum) < 0) {
                //if n less than powerNum tooHigh = guess
                tooHigh.copyFrom(guess);
            } else {
                //else lowEnough = guess
                lowEnough.copyFrom(guess);
            }
            //update the difference at the end of each iteration
